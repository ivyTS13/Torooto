...
export default function PileDrawer() {
  ...
  if (isFetchingDeck) {
    return (
      <div className="h-full bg-[#030014] flex flex-col items-center justify-center gap-4 min-h-[50vh]">
  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden bg-[#030014] text-white rounded-2xl">
      <SettingsPanel />

      {error && (
        <div className="absolute top-8 left-0 right-0 flex justify-center z-40 px-4">
      ...
      {/* Main Board Area – now scrollable if needed */}
      <div className="flex-1 flex flex-col items-center justify-start overflow-y-auto pt-16 pb-24 md:pb-36 px-4">
        {!hasDrawnCards ? (
          <motion.div
      ...
      {/* Interactive Deck – remains fixed at bottom */}
      <AnimatePresence>
        {!isDonePicking && (
          <motion.div
            ...
            className="absolute bottom-0 w-full flex justify-center pb-8 pointer-events-none"
          >
